...
//--------------ex01 과제요구사항-------start-
void Bureaucrat::signForm(Form &fInfo)
{
	if(fInfo.getIsSigned() == true)
	{
		std::cout << this->getName() << " signed " << fInfo.getName() << std::endl;
	}
	else
	{
		std::cout << this->getName() << " couldn't sign " << fInfo.getName() << " because " << std::endl;
